...
class Spelling extends StatefulWidget {
  const Spelling({super.key, required this.topic});
  final Topic topic;
  @override
  State<StatefulWidget> createState() => SpellingState(topic);
}

class SpellingState extends State<Spelling> with TickerProviderStateMixin {
  late AnimationController controller;
  TextEditingController _textController = TextEditingController();
  List<Word> listWord = [];
  SpellingState(this.topic);
  final Topic topic;
  int timeLeft = 5;
  int clock = 0;
  ...
  Widget _buildStartScreen() {
    return Column(
      ...
      children: [
        ...
        FutureBuilder(
          future: getResultByTopicIDAndType(topic.id, "spelling"),
          builder: (context, snapshot) {
            if (snapshot.hasError) {
              return Center(
                child: Text("Error in loading reault: ${snapshot.error}"),
              );
            }
            if (snapshot.hasData) {
              if (snapshot.data != null) {
              ...
              child: TextField(
                ...
                decoration: InputDecoration(
                  suffixIcon: IconButton(
                    ...
                    onPressed: () {
                      setState(() {
                        calScore(_textController.text.toString());
  ...
  void calScore(String choice) {
    if (choice.toLowerCase() == answer[indexWord].toLowerCase()) {
      score++;
    }
